...
public interface SavedPostRepository extends JpaRepository<SavedPost, SavedPostId> {
    @Query(value = """
        SELECT sp.user_id, sp.post_id
        FROM saved_post sp
        JOIN post p ON p.id = sp.post_id
        JOIN user u ON u.id = sp.user_id
        JOIN post_feelings pf ON pf.post_id = p.id
        WHERE (pf.feeling IN (:feelings))
        AND (:day IS NULL OR DAY(p.post_time) = :day)
        AND (:month IS NULL OR MONTH(p.post_time) = :month)
        AND (:year IS NULL OR YEAR(p.post_time) = :year)
